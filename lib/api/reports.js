  ...
  try {
    ...
    if (filters.validationStatus && filters.validationStatus !== 'all') {
      if (filters.validationStatus === 'validated') {
        // Include both AI validated and staff approved reports
        query = query.in('validation_status', ['validated', 'automatically_valid', 'approved'])
      } else {
...
export async function updateLifecycleStage(reportId, lifecycleStage) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/reports/${reportId}/lifecycle`, {
      method: 'PATCH',
      headers,
...
export async function updateReportDetails(reportId, details) {
  try {
    const API_BASE_URL = process.env.NEXT_PUBLIC_BACKEND_API_URL
    const token = localStorage.getItem('authToken')
    ...
    if (!response.ok) {
      const errorData = await response.json()
      throw new Error(errorData.message || 'Failed to update report details')
    }
