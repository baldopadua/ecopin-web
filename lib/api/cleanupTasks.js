...
export async function fetchCleanupTasks(assignedToMe = false) {
  try {
    ...
    const url = new URL(`${API_BASE_URL}/api/cleanup-tasks`)
    if (assignedToMe) {
      url.searchParams.append('assigned_to_me', 'true')
    }
...
export async function fetchAvailableCrew() {
  try {
    const API_BASE_URL = process.env.NEXT_PUBLIC_BACKEND_API_URL
    const token = localStorage.getItem('authToken')
    ...
    const response = await fetch(`${API_BASE_URL}/api/cleanup-tasks/available-crew`, {
      headers
    })
    ...
    if (!response.ok) {
      const errorData = await response.json()
      console.error('Failed to fetch available crew:', errorData)
      throw new Error(errorData.message || 'Failed to fetch available crew')
    }

    const data = await response.json()
    console.log('Available crew fetched successfully:', data)
    return data
  } catch (error) {
...
export async function assignCleanupTask(taskId, assignedCrewIds) {
  try {
    const API_BASE_URL = process.env.NEXT_PUBLIC_BACKEND_API_URL
    const token = localStorage.getItem('authToken')
    ...
    const response = await fetch(`${API_BASE_URL}/api/cleanup-tasks/${taskId}/assign`, {
      method: 'PATCH',
      headers,
      body: JSON.stringify({ assigned_crew_ids: assignedCrewIds })
    })
    ...
    if (!response.ok) {
      const errorData = await response.json()
      console.error('Failed to assign cleanup task:', errorData)
      throw new Error(errorData.message || 'Failed to assign cleanup task')
    }

    const data = await response.json()
    console.log('Cleanup task assigned successfully:', data)
    return data
  } catch (error) {
